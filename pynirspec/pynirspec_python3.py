...
class Environment():
	...
	def __init__(self,settings_file=None,detpars_file=None,nirspec=1):
		...
		if settings_file is None:
			...
			self.settings.read(sys_dir+'/'+'nirspec.ini') # outdated
		else:
...
class Observation():
	...
	def getSetting(self):
		...
		#print(echelles,crossdisps)
		#return 'KL_EXOPL', float(echelles[0]), float(crossdisps[0])	
		...
		#print(echelles, crossdisps)	
		#print(echelle, crossdisp)
		#print(sub)	
		#print(self.Envi.getSections()[sub[0]])
		
		#### SOMETIMES the echelle and cross-disp weren't written to the header correctly (esp in NIRSPEC1)
		try:
			return self.Envi.getSections()[sub[0]],echelle,crossdisp
		except IndexError:
			return self.Envi.getSections()[0], float(echelles[0]), float(crossdisps[0])
	...
	def divideInStack(self,Obs):
		...
		cube2 = np.zeros((self.stack.shape[2], self.stack.shape[0], self.stack.shape[1]))
...
class Flat(Observation):
	def __init__(self,filelist,dark=None,norm_thres=5000.,save=False,sci_tname=None,out_dir=None,nirspec=2,**kwargs):
		...
		# close files opened in Observation
		for plane in self.planes:
			#print("closing file:",plane)
			plane.close()
...
class Nod(Observation):
	def __init__(self,filelist,dark=None,flat=None,badpix='badpix.dmp',plots_dir=None,tname=None,nirspec=2, firstframe='A',**kwargs):
		...
		pairs = self._getPairs(RAs,DECs,FileNums,firstframe)
		...
		#	im = self.ustack[:, :, slice]
		#	cube[slice, :, :] = im
		#pf.writeto(self.plots_dir + '/A-B-after-bpRemoval-unc.fits', cube, overwrite=True)
	...
	def _getPairs(self,RAs,DECs,FileNums,firstframe='A'):
		nexp = len(RAs)				   
		...
		try:
			...
			problem = False
		### this is to fix issues where the ra and dec positions weren't written into header ('#### Error ####')
		except TypeError:
			problem = True	
		...
		if not problem:
			...
			print('')
			print('%7s %12s %12s %5s' % ('Exp.', 'RA', 'DEC', 'A or B'))
		...
		if (problem):
			...
			for i in range(0,len(RAs)):
				try:
					...
					print  ('{} {} {} {}'.format(FileNums[i], RAs[i], DECs[i], AorB[i]))
			print ('\n')
...
class Order():
	def __init__(self,Nod,onum=1,trace=None,write_path=None, targetType = 'bright'):
		...
		if oldskyrectway:
			...
			#	cube[slice, :, :] = im
			pf.writeto(plots_path, self.sky_rect, overwrite=True)
			#plt.show()
		...
		newskyrectway = False
		if newskyrectway:
			#beamSkyRect has the rectified A and B images
			rectifiedA = beamSkyRect[0]
			rectifiedB = beamSkyRect[1]
			urectifiedA = beamUSkyRect[0]
			urectifiedB = beamUSkyRect[1]
			shape = np.shape(rectifiedA)
			height = shape[0]
			
			bottomofrectifiedA = rectifiedA[:int(height / 2)]		#[int(npsf/2):-1]
			topofrectifiedB = rectifiedB[int(height/2):] 	#[:int(npsf / 2) - 1]
			ubottomofrectifiedA = urectifiedA[:int(height / 2)] 		#[int(npsf/2):-1]
			utopofrectifiedB = urectifiedB[int(height/2):]	#[:int(npsf / 2) - 1]
			
			self.sky_rect = np.concatenate([bottomofrectifiedA, topofrectifiedB])
			self.usky_rect = np.concatenate([ubottomofrectifiedA, utopofrectifiedB])
			...
			plots_path = self.write_path.replace('SPEC2D', 'PLOTS/') + object + '_' + date + '_' + time + 'skyrect_final_CBversion-order' + str(onum) + '.fits'
			pf.writeto(plots_path, self.sky_rect, overwrite=True)			
		...
		#plt.figure()
		#xx = np.sum(self.image_rect,axis=1)
		#for num,ii in zip(xx,self.image_rect):
		#	if num == np.max(xx):
		#		maxcounts = ii
		#	if num == np.min(xx):
		#		mincounts = ii
		#filename = '/home/cbuzard/WhiteDwarfBrownDwarf/NLTT_counts_fromrectimage_20200107_81-82.dat'
		#col1 = np.arange(len(mincounts))
		#col2 = maxcounts
		#col3 = mincounts
		#header=[str(0), str(0), str(0), filename]
		#f=open(filename, 'w')
		#f.write(header[0]+'\t'+header[1]+'\t'+header[2]+'\t'+header[3] +'\n')
		#for col_one, col_two, col_three in zip(col1, col2, col3) : 
		#	f.write('%-10f %10f %10f' % (col_one, col_two, col_three)+'\n')		
		#f.close()
		...
		#	cube[slice, :, :] = im
		pf.writeto(plots_path, self.image_rect, overwrite=True)
		#plt.show()
		#assert(1==0)
	...
	def fitTrace(self, image, OneOrTwo, kwidth=100, fitMethod = 'FFT'):
		...
		#plt.figure()
		#plt.plot( np.arange(sh[1]), centroids)
		#plt.show()
		...
		#plt.plot(xindex, centroids)
		#plt.plot(xindex[gsubs],poly(xindex[gsubs]))
		#plt.figure()
		#plt.plot(xindex[gsubs],totals[gsubs])
		#plt.show()
		#plt.close()
		#assert(1==0)
...
class Spec1D():
	def __init__(self,Order,sa=True,write_path=None,nirspec=1):
		...
		#plt.figure()
		#plt.plot(self.sky_pos)
		##plt.figure()
		#plt.plot(self.sky_neg)
		#plt.show()
	...
	def extract(self,PSF,method='single'):
		...
		## take away edge of order bit
		pos_psf[np.logical_or((pos_y < (x0_pos - 20.*sigma_pos)),(pos_y > (x0_pos + 20.*sigma_pos)))] = 0.0
		neg_psf[np.logical_or((neg_y < (x0_neg - 20.*sigma_neg)),(neg_y > (x0_neg + 20.*sigma_neg)))] = 0.0
		...
		plt.savefig(self.writepath.replace('SPEC1D', 'PLOTS/') + object + '_' + date + '_' + time + 'posPSF-order' + str(
				self.onum) + '.png')
		plt.close()
		...
		ax.set_xlabel('y index')
		ax.set_ylabel('normalized flux')
		plt.savefig(self.writepath.replace('SPEC1D', 'PLOTS/') + object + '_' + date + '_' + time + 'negPSF-order' + str(
			self.onum) + '.png')
		plt.close()	
		...
		for i in np.arange(sh):
			...
			#flux_pos[i] = (im[:int(npsf/2)-1,i]).sum()
			#flux_neg[i] = (im[int(npsf/2):-1,i]).sum()
			## uniform weights
			#flux_pos[i] = (pos_psf[:int(npsf/2)-1]*im[:int(npsf/2)-1,i]).sum() / (pos_psf[:int(npsf/2)-1]**2).sum()
			#flux_neg[i] = (neg_psf[int(npsf/2):-1]*im[int(npsf/2):-1,i]).sum() / (neg_psf[int(npsf/2):-1]**2).sum()
		...
		#
		#plt.figure()
		#plt.imshow(pos_psf*im[:int(npsf/2)-1], cmap='gray')
		#plt.colorbar()
		#plt.text(30, 30, 'pos')
		#
		#plt.figure()
...
#		plt.plot(neg_psf)
		...
		#ax[0].plot(self.wave_pos, flux_pos, drawstyle ='steps-mid',label='Pos flux w/ initial wavelength fit')
		#ax[1].plot(self.wave_pos, sky_pos, drawstyle='steps-mid', label='Pos sky w/ initial wavelength fit')
		ax[0].plot(flux_pos, drawstyle ='steps-mid',label='Pos flux')
		ax[1].plot(sky_pos, drawstyle='steps-mid', label='Pos sky')
		...
		fig, ax = plt.subplots(2,1)
		ax[0].plot(flux_neg, drawstyle='steps-mid', label='Neg flux')
		ax[1].plot(sky_neg, drawstyle='steps-mid', label='Neg sky')
		ax[0].legend(loc='upper left')
		ax[1].legend(loc='upper left')
...
class WaveCal():
	...
	def _updateWave(self,specfile,WavePos,WaveNeg):
		...
		fig, ax = plt.subplots(2,1)
		ax[0].plot(spec1d_ini_wfit_pos['wave'], spec1d_ini_wfit_pos['flux'], label='pos flux')
		ax[1].plot(spec1d_ini_wfit_pos['wave'], spec1d_ini_wfit_pos['Radiance'], label='pos rad')
		ax[0].legend(loc='upper left')
		ax[1].legend(loc='upper left')
		...
		fig, ax = plt.subplots(2,1)
		ax[0].plot(spec1d_ini_wfit_neg['wave'], spec1d_ini_wfit_neg['flux'], label='neg flux')
		ax[1].plot(spec1d_ini_wfit_neg['wave'], spec1d_ini_wfit_neg['Radiance'], label='neg rad')
		ax[0].legend(loc='upper left')
		ax[1].legend(loc='upper left')
		plt.savefig(self.path + '/waveNeg' + onum + '.png')
...
class CalSpec():
	...
	# remove regions of low transmission
...
class Reduction():
	...
	def __init__(self,flat_range=None, flat_dark_range=None, dark_range=None,
		...
		print ('Output directory for target:',out_dir)
		print ()
		...
		# include standard files if called
		if std_range:
			self.tdict = {'science':self.sci_names1,'standard':self.std_names}
			self.ndict = {'science':sci_tname,'standard':std_tname}
		...
		if level2:
			print("### Starting Reduction from level 2")
			self._level2()
		...
		for key in self.tdict.keys():
			...
			try:
				ONod	= Nod(self.tdict[key],flat=OFlat,dark=ODark, tname=self.ndict[key], SettingsFile=self.SettingsFile,
							  plots_dir = self.plots_path, badpix=self.out_dir+'/badpix.dmp',nirspec=self.nirspec, firstframe=self.firstframe)
