...
class Nod(Observation):
	def __init__(self,filelist,dark=None,flat=None,badpix='badpix.dmp',plots_dir=None,tname=None,nirspec=2,**kwargs):
...
class Order():
	def __init__(self,Nod,onum=1,trace=None,write_path=None, targetType = 'bright'):
		...
		if trace is None:
			yr1, trace1 = self.fitTrace_all(self.image1, 1, fitMethod=fitMethod)
	...
	def fitTrace(self, image, OneOrTwo, kwidth=100, fitMethod = 'FFT'):
		...
		gsubs_cut = gsubs[0][np.where(gsubs[0]>175)]
		gsubs_cut = gsubs_cut[np.where(gsubs_cut<1273)]
	...
	def fitTrace_all(self, image, OneOrTwo, kwidth=100, fitMethod = 'FFT'):
		...
		#yindex = np.arange(yr[0], yr[1] + 1)  # Top or Bottom of Order
		yr = (0, sh[0])
		yindex = np.arange(0, sh[0])  # Top or Bottom of Order
		yindex = yindex.astype(np.int64)
		kernel = np.median(image[yindex, int(sh[1] / 2) - kwidth:int(sh[1] / 2) + kwidth], 1)
		centroids = []
		totals = []
